...
if (settings.msToProcess < 36000) || (sum([trackResults.status] ~= '-') < 4)
    ...
    msgbox('Record is to short or too few satellites tracked. Exiting!');
    navSolutions = [];
    eph          = [];
    return
end
...
if (isempty(activeChnList) || (size(activeChnList, 2) < 4))
    ...
    msgbox('Too few satellites with ephemeris data for postion calculations. Exiting!');
    navSolutions = [];
    eph          = [];
    return
end
...
%% Coordinate conversion ==================================================
        ...
        msgbox(['   Measurement No. ', num2str(currMeasNr), ...
